...
微信在 macOS 上没有提供导出或备份聊天记录的官方入口。你的消息、图片、语音和文件只存在本地一个加密数据库里——无法整体导出，无法跨全部历史检索，也没有任何程序化访问自己数据的方式。

这带来真实的困扰：换机或重装系统可能让多年的聊天记录付诸东流，且无官方手段找回；你无法归档或全文检索自己的历史记录；你产生的数据实际上并不掌握在自己手里。WXDecipher 正是为解决这些问题而生——让你在**自己的 Mac** 上、以只读方式解锁并读取**自己**的微信本地数据，从而备份、检索并真正掌控它们，全程离线。
