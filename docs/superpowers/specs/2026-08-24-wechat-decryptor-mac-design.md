...
原项目为开源的 **Windows 微信聊天记录解密工具**（`package_version 0.7.0`，PySide6 + Qt Quick/QML + PyInstaller 打包）。源码已丢失，仅剩：

- PyInstaller 发行版：`微信解密-8月18日23点57/`（两个 exe + `_internal` 依赖 + `runtime/wx_key.dll`）。
- 设计文档：`README_解密全过程.md`、`README_Qt界面.md`、`README_便携版.md`、`README_小白工具.md`、`README_兼容性检查.md`、`README_MCP本地调用.md`。
- QML 界面源码（打包在 `_internal/ui_qt/qml/`）：`FullMain.qml`、`Main.qml`、`components/MessageBubble.qml`（**可直接复用**）。

本项目按上述设计文档 + QML，重建一套面向 macOS 的实现。

## 2. 目标与非目标

### 2.1 目标

1. 在 macOS 复刻 Windows 版价值链：**发现账号 → 获取/复用密钥 → 解密数据库 → 只读查看聊天与媒体 → 本地 MCP 服务**。
